"""基于 reverse_nordic p1/p2 参数的实时计数叠加视频测试."""
...
CONFIG_PATH = PROJECT_ROOT / "config" / "action_configs" / "reverse_nordic_trained.json"
SPLIT_PATH = PROJECT_ROOT / "data" / "datasets" / "reverse_nordic_v1.0_split.json"
OUTPUT_DIR = PROJECT_ROOT / "output" / "tests"
OUTPUT_PATH = OUTPUT_DIR / "reverse_nordic_test_count_overlay.mp4"
...
def test_reverse_nordic_realtime_count_overlay_video_generation() -> None:
    ...
    # 严格使用 config_data 中训练生成的 count_layer
    count_layer = config_data.get("count_layer", {})
    if not count_layer:
        pytest.fail("配置中找不到 count_layer")

    # 为了增加鲁棒性，使用聚合后的 param_ci 替换严格的中位数 thresholds (如果有)
    # 因为真实的阈值往往比严格中位数要宽泛
    param_ci = count_layer.get("aggregation", {}).get("param_ci", {})
    if param_ci:
        # valley_to_peak_to_valley: 数值从小变大再变小
        # enter_p1(数值增大触发), 取 param_ci 下界更易触发
        # exit_p1(数值增大触发), 取 param_ci 下界更易触发
        # enter_p2(数值减小触发), 取 param_ci 上界更易触发
        # exit_p2(数值减小触发), 取 param_ci 上界更易触发
        # 这里为了尽可能记录所有动作，我们将使用放宽的边界。
        count_layer["thresholds"] = {
            "enter_p1": param_ci.get("enter_p1", [count_layer["thresholds"]["enter_p1"]])[0],
            "exit_p1": param_ci.get("exit_p1", [count_layer["thresholds"]["exit_p1"]])[0],
            "enter_p2": param_ci.get("enter_p2", [count_layer["thresholds"]["enter_p2"]])[1],
            "exit_p2": param_ci.get("exit_p2", [count_layer["thresholds"]["exit_p2"]])[1],
        }
