"""基于 bulgarian_squat p1/p2 参数的实时计数叠加视频测试."""
...
CONFIG_PATH = PROJECT_ROOT / "config" / "action_configs" / "bulgarian_squat_trained.json"
SPLIT_PATH = PROJECT_ROOT / "data" / "datasets" / "bulgarian_squat_v1.0_split.json"
OUTPUT_DIR = PROJECT_ROOT / "output" / "tests"
OUTPUT_PATH = OUTPUT_DIR / "bulgarian_squat_test_count_overlay.mp4"
...
def test_bulgarian_squat_realtime_count_overlay_video_generation() -> None:
    ...
    # 手动干预：使用 knee_flexion 作为主控指标
    # 注意：膝盖弯曲（knee_flexion）的定义是大小腿的夹角。
    # 站立时腿是直的，角度大（约 140~170度）。
    # 下蹲时腿折叠，角度小（约 50~90度）。
    # 因此动作是从 大角度 -> 小角度 -> 大角度。这属于 falling_to_valley_to_rising，
    # 即 polarity 应当设定为 peak_to_valley_to_peak。
    count_layer = {
        "control_metric": "knee_flexion",
        "polarity": "peak_to_valley_to_peak",  
        "thresholds": {
            "enter_p1": 118.0,   # 开始下蹲，角度跌破 118度
            "exit_p1": 110.0,    # 蹲到底，角度跌破 110度 (针对保加利亚蹲前腿无法完全伸直的特性)
            "enter_p2": 112.0,   # 开始起立，角度回升过 112度
            "exit_p2": 118.0     # 站直，角度回升过 118度
        },
        "timing": config_data.get("count_layer", {}).get("timing", {})
    }
