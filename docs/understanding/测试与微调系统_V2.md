...
> 
## 2026-04 task1/task2 当前实现状态

> 本节描述当前仓库已经落地的最小闭环。下文较早的“参数迭代系统”内容可继续作为设计背景阅读，但以本节为当前实现基线。
...
1. 按 `action_id + label` 自动拆分 `train / validation / test`
2. 测试集低置信样本自动打 `confusing_sample`
3. 误判样本自动进入 `queued_for_retraining`
4. 多轮误判样本自动进入 `pending_annotation`
5. 仓储支持 JSON 持久化与恢复
...
1. `pending / running / succeeded / failed / cancelled` 状态机
2. 最大重试次数控制
3. 3 类触发器
   - 新样本数达到 20
   - 低置信样本达到 10
   - 超过 24 小时未训练
4. baseline / candidate 统一评估
5. 版本注册、发布、回滚、历史记录
6. candidate 评估结果自动喂给 feedback loop
